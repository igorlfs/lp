...
fun amount (Centavos x) = x
  | amount (Reais x) = Real.round (x * 100.0)
  | amount (Pessoa_Dinheiro (_,x)) = Real.round (x * 100.0);
...
(* Exercício 10 *)
datatype Planeta = Mercurio | Venus | Terra | Marte | Jupiter | Saturno | Urano
                 | Netuno;
...
fun sumAll Leaf = 0
  | sumAll (Node(left,x,right)) = sumAll left + x + sumAll right;
val t = Node (Node (Leaf, 1, Leaf), 6, Node (Leaf, 12, Leaf));
sumAll t;

(* Exercício 12 *)
fun multiPairs (x,y) = ListPair.map (fn (x,y) => x*y) (x,y);
